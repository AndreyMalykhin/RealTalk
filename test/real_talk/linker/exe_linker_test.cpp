...
TEST_F(ExeLinkerTest, Link) {
...

  {
    ...
    cmds_code->Write<CmdId>(CmdId::kCreateGlobalDoubleVar);
    vector<uint32_t> var_index_placeholders;
    var_index_placeholders.push_back(cmds_code->GetPosition());
    cmds_code->Write<uint32_t>(numeric_limits<uint32_t>::max());
    cmds_code->Write<CmdId>(CmdId::kLoadGlobalLongVarValue);
...

  {
    ...
    var_index_placeholders2.push_back(cmds_code->GetPosition());
    cmds_code->Write<uint32_t>(numeric_limits<uint32_t>::max());
    cmds_code->Write<CmdId>(CmdId::kLoadGlobalDoubleVarValue);
    vector<uint32_t> var_index_placeholders;
    var_index_placeholders.push_back(cmds_code->GetPosition());
    cmds_code->Write<uint32_t>(numeric_limits<uint32_t>::max());
    cmds_code->Write<CmdId>(CmdId::kLoadFuncValue);
  ...
  cmds_code->Write<uint32_t>(var_index2);
  cmds_code->Write<CmdId>(CmdId::kLoadFuncValue);
  vector<uint32_t> func_address_placeholders2;
  func_address_placeholders2.push_back(cmds_code->GetPosition());
